...
import com.capinfo.core.filter.AppAuthenticationFilter;
...
//    RetryLimitCredentialsMatcher rm = new RetryLimitCredentialsMatcher(getCacheManager(),2);
    ...
    @Bean(name = "appAuthorizingRealm")
    public AppAuthorizingRealm getAppAuthorizingRealm() {
        AppAuthorizingRealm realm = new AppAuthorizingRealm();
        return realm;
    }
    @Bean
    ...
    public SecurityManager getSecurityManager(@Qualifier("loginRealm") LoginRealm loginRealm,
                                              @Qualifier("appAuthorizingRealm")AppAuthorizingRealm appAuthorizingRealm) {
        DefaultWebSecurityManager dwm = new DefaultWebSecurityManager();
        dwm.setAuthenticator(getCustomeModularRealmAuthenticator());
        List<Realm> realms = new ArrayList<Realm>();
        realms.add(loginRealm);
        realms.add(appAuthorizingRealm);
    ...
    @Bean
    public AppAuthenticationFilter getAppAuthenticationFilter() {
        AppAuthenticationFilter aaf = new AppAuthenticationFilter();
        return aaf;
    }
    @Bean
